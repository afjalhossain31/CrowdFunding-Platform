...
export default function ExploreCampaignsPage() {
  ...
  const itemsPerPage = 6; // প্রতি পেজে ক্যাম্পেইনের সংখ্যা
    ...
    <div className="min-h-screen bg-white dark:bg-zinc-950 font-sans pb-20">
      ...
      <div className="relative bg-slate-900 text-white py-24 px-4 text-center overflow-hidden mb-12">
        ...
        <div className="relative z-10 max-w-4xl mx-auto">
          <h1 className="text-4xl md:text-5xl font-black tracking-tight mb-3">All Campaigns</h1>
          <p className="text-sm md:text-base text-slate-300 font-medium">
            <Link href="/" className="hover:text-amber-400 transition">Home</Link> <span className="mx-2">/</span> <span className="text-amber-400">Campaigns</span>
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-10 pb-4 border-b border-slate-200 dark:border-zinc-800 gap-4">
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Showing <span className="font-bold text-slate-800 dark:text-white">{sortedCampaigns.length > 0 ? indexOfFirstItem + 1 : 0}–{Math.min(indexOfLastItem, sortedCampaigns.length)}</span> of {sortedCampaigns.length} results
          </p>
        ...
        {loading ? (
          <div className="text-center py-20">
            <span className="loading loading-spinner loading-lg text-amber-500">Loading all campaigns...</span>
          </div>
        ) : currentCampaigns.length === 0 ? (
          <p className="text-center text-gray-500 py-16 text-sm">No campaigns available at the moment.</p>
        ) : (
                  ...
                  <div className="p-6 flex-1 flex flex-col justify-between">
                    <div>
                      <p className="text-xs text-emerald-600 dark:text-emerald-400 font-semibold mb-1">
                        by {campaign.creator_name || "Creator"}
                      </p>
